...
public class Deck {
    ...
    public Deck() { // constructor
        int numberOfCards = (Card.MaxSuit + 1 - Card.MinSuit) * (Card.MaxValue + 1 - Card.MinValue); // number of cards
                                                                                                     // in total
        deck = new Card[numberOfCards]; // creates a deck of
        var i = 0;
        for (int suit = Card.MinSuit; suit <= Card.MaxSuit; suit++) {
            for (int value = Card.MinValue; value <= Card.MaxValue; value++) { // creates cards to fill up the deck, one
                                                                               // of each
